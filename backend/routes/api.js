...
class SiyadahAPI {
  ...
  chat(tenantId, message) {
    ...
    return {
      ...
      steps: (result.flow.steps||[]).map(function(s) { return { piece: s.piece_id||"unknown", action: s.action_name||"", role: s.role||"" }; }),
      trigger: { piece: (result.flow.trigger||{}).piece_id||"webhook", type: (result.flow.trigger||{}).trigger_name||"catch_webhook" },
